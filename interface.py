...
def y2(x, t):
	return 1 + sin(x*t) - cos(4*x*t)
def u2(x, t):
	return x*cos(t*x) + k*t**2 *sin(t* x) - 4*x*sin(4*x*t) + k*16*t**2 *cos(4*x*t)
...
def G1(x, t):
	return np.exp(-x**2 / (4*k *t))/(2*np.sqrt(Pi*t*k)) if t > eps else 0
def G2(x, y): # laplace
	#return ln(x**2 + y**2)/(4* Pi)
	return log(x ** 2 + y ** 2) / (4 * Pi)
def G3(x, t): # wave 
...
class Index_Funcs(object):
	...
	class DotBuilder:
		def __init__(self, dots, a, b, T):
			...
			self._markedx = []
			self._markedy = []
			__far_left = a - (b-a)*0.4
			__far_right = b + (b-a)*0.4
			__far_down = - T*1.4
	...
	"""Opens a window, that lists avaliable operators, and gives opportunity to choose one"""
	...
	def evaluateB(self, event):
		...
		self.operator = 0
		self.update_text()
		self.a, self.b, self.T = 1, 2, 3

		self._main_plot_ax.clear()
		self._main_plot_ax.set(xlabel = "X", ylabel = "T", zlabel = "y(x, t)")
		...
		X = np.arange(self.a, self.b, h_x)
		Y = np.arange(0, self.T, h_t)
		X, Y = np.meshgrid(X, Y)
...
class Window:
	...
	def __init__(self, fig, ax, figsize = (1, 1)):
		...
		self.__oper_text = plt.text(-8.15, 12.7, "Lu = 0", fontsize=15, style='italic')
		self.__func_text = plt.text(-8.15, 9.75, "y(x, t) = 0", fontsize=15, style='italic')
	...
	def _init_fields_(self):
		# * 1st field
		self._operator_b = widg.Button(self._choose_operator, r'Обрати оператор L', color = '#ff9900')
		self._operator_b.on_clicked(self.callback.operator_listbox)
		# * 2nd field
		self._function_b = widg.Button(self._choose_function, "Обрати функцію y(x, t)", color = '#ff9900')
		self._function_b.on_clicked(self.callback.function_listbox)
		# * 3rd field
		plt.text(-8.2, 8.25, "Введіть значення часу T для", fontsize=15, style='italic')
		plt.text(-8.2, 7.6, "  інтервалу [0, T]:", fontsize=15, style='italic')
		self._valueT_txtbox = widg.TextBox(self._T_field, "T = ", initial="0")
		self._valueT_txtbox.on_submit(self.callback.submit_T)
		plt.text(-8.2, 5.85, "Введіть значення границі спостереження", fontsize=15, style='italic')
		plt.text(-8.2, 5.3, "  a та b інтервалу [a, b]:", fontsize=15, style='italic')
